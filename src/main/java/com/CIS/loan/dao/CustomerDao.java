...
import com.CIS.loan.model.loan;
...
//Customer 
	...
	public int saveCustomer(customer c) {
		...
		String sql = "insert into loan.customer(fullname,dateOfBirth,email,phone,password) values('" + c.getFullName()
				+ "','" + c.getDateOfBirth() + "','" + c.getEmail() + "'," + c.getPhone() + ",'" + c.getPassword()
				+ "')";
	...
	public List<customer> getCustomer() {
		return template.query(
				"SELECT fullName, dateOfBirth, loanBalance, userAmount, installmentPlan FROM loan.customer JOIN loan.loanDetails ON loan.customer.customerId = loan.loanDetails.customerId;",
...
//Inventory
	...
	public int save(inventory i) {
		String sql = "insert into loan.inventory(productName,price,qty,description,category,brand) values('"
				+ i.getProductName() + "'," + i.getPrice() + "," + i.getQty() + ",'" + i.getDescription() + "','"
				+ i.getCategory() + "','" + i.getBrand() + "')";
		return template.update(sql);
	}

	public int updateInventory(inventory i) {
		String sql = "update loan.inventory set productName='" + i.getProductName() + "', price=" + i.getPrice()
				+ ",qty='" + i.getQty() + "',description='" + i.getDescription() + "',category='" + i.getCategory()
				+ "',brand='" + i.getBrand() + "' where productId=" + i.getProductId() + "";
		return template.update(sql);
	}
...
//Category
	...
	public int save(Category c) {
		String sql = "insert into loan.category(categoryId,categoryName) values('" + c.getCategoryId() + "','"
				+ c.getCategoryName() + "')";
		return template.update(sql);
	}

	public int updateCategory(Category c) {
		String sql = "update loan.category set categoryName='" + c.getCategoryName() + "' where categoryId="
				+ c.getCategoryId() + "";
		return template.update(sql);
	}
